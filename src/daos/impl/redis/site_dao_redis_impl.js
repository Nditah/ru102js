...
const findAll = async () => {
  // START CHALLENGE #1
  const client = redis.getClient();
  const siteKeyArray = await client.smembersAsync(keyGenerator.getSiteIDsKey());
  const result = await Promise.all(siteKeyArray.map(async (siteKey) => {
    const siteHash = await client.hgetallAsync(siteKey);
    return (siteHash === null ? siteHash : remap(siteHash));
  }));
  // console.log({ siteKeyArray, result });
  return result;
  // END CHALLENGE #1
};
/* eslint-enable */
